...
class denoiser(object):
    def __init__(self, sess, stride=128, input_c_dim=1, batch_size=4):
        ...
        self.X_input = tf.placeholder(tf.float32, [None, None, None, self.input_c_dim],
                                 name='real_image')
        self.X_input_inv = tf.placeholder(tf.float32, [None, None, None, self.input_c_dim], name='real_image_inv')
        ...
        def true_fn_inv():
           s = tf.zeros(shape=tf.shape(self.X_input_inv))
           for k in range(0, L):
               gamma = (tf.abs(tf.complex(tf.random_normal(shape=tf.shape(self.X_input_inv), stddev=1),
                                          tf.random_normal(shape=tf.shape(self.X_input_inv), stddev=1))) ** 2) / 2
               s = s + gamma
           s_amplitude = tf.sqrt(s / L)
           log_speckle = tf.log(s_amplitude)
           log_norm_speckle = log_speckle / (M - m) 

           return tf.add(self.X_input_inv, log_norm_speckle) 
        ...
        def false_fn_inv():
           return self.X_input_inv



        self.X = tf.cond(self.is_training, true_fn, false_fn) 
        self.X_inv = tf.cond(self.is_training, true_fn_inv, false_fn_inv)

        with tf.variable_scope("model"):
            self.Y, self.Y2 = autoencoder(self.X, self.input_c_dim) # MAP PREDICTION, USE THE CORRECT OUTPUT OF u_net.py
        with tf.variable_scope("model", reuse=True):
            self.Y1, self.Y2_2 = autoencoder(self.X_inv, self.input_c_dim) # MAP PREDICTION, USE THE CORRECT OUTPUT OF u_net.py
        ...
        self.loss = self.alpha*((1.0 / batch_size) * tf.reduce_sum( tf.abs( self.X_input[:,:,:,:1] - self.Y+cn))) + self.beta * ((1.0/batch_size)*tf.reduce_sum(tf.abs(self.Y2 - self.Y + self.X_input[:,:,:,-1:]))) + self.gamma * ((1.0/batch_size) * tf.reduce_sum(tf.abs(self.Y2 - self.Y2_2)))
...
####################################################################   
    ...
    def evaluate(self, iter_num, test_data, eval_files, eval_set, sample_dir):
        ...
        for idx, current_test in enumerate(test_data):
            ...
            pile_im = real_image[:,:256,:256,:].copy()
            idx_lst=[idx]
            real_image_channels_inv=real_image[:,:256,:256,:] # T1

            ## PILE
            for channel_i in range(self.input_c_dim-1):
                pile_im = test_data[(idx+1)%len(test_data)]  # WE TAKE TIME T2 THAT IS THE NEXT IMAGE IN THE LIST.
                real_image_channels = np.concatenate((real_image_channels, pile_im[:,:256,:256,:]), axis=3)                   ##T1T2 STACK
                real_image_channels_inv = np.concatenate((pile_im[:,:256,:256,:], real_image_channels_inv), axis=3)     ##T2T1 STACK

            output_clean_image, output_clean_image2, output_clean_image2_2, noisy_image = self.sess.run(
                [self.Y, self.Y2, self.Y2_2, self.X],
                feed_dict={self.X_input: real_image_channels, self.X_input_inv: real_image_channels_inv,
                           self.is_training: False})
            groundtruth = denormalize_sar(real_image[:, :256, :256, :])
            noisyimage = denormalize_sar(noisy_image[:,:,:,:1])
            noisyimage2 = denormalize_sar(noisy_image[:,:,:,-1:])
            ...
            imagename_map = imagename0.replace('.npy', '_map1_' + str(iter_num) + '.npy')
            save_mapbm(output_clean_image2[0,:,:,0]*255, imagename_map, sample_dir)
            imagename_map = imagename0.replace('.npy', '_map2_' + str(iter_num) + '.npy')
            save_mapbm(output_clean_image2_2[0,:,:,0]*255, imagename_map, sample_dir)
            
            print("Loss")
            print(np.sum(np.abs(output_clean_image2 - output_clean_image2_2)))
...
####################################################################   
        ...
        batch_images_inv = np.zeros((batch_size, pat_size, pat_size, self.input_c_dim))
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
                ...
                for i in range(batch_size):
                    ...
                    im_channels_inv = np.expand_dims(im_channels_inv, axis=2) # T1


                    ## PILE
                    if self.input_c_dim == 2 :
                        im1 = data[id_pile][1][ x:x + pat_size, y:y + pat_size, (id_date+1)%data[id_pile][1].shape[-1]]   # WE TAKE TIME T2 THAT IS THE NEXT IMAGE IN THE LIST.
                        
                        im_channels = np.concatenate((im_channels, np.expand_dims(im1, axis=2)), axis=2)                      ##T1T2 STACK
                        im_channels_inv = np.concatenate((np.expand_dims(im1, axis=2), im_channels_inv), axis=2)        ##T2T1 STACK

                    batch_images[i,:,:,:] = im_channels
                    batch_images_inv[i,:,:,:] = im_channels_inv


             
                _, loss,_= self.sess.run([self.train_op, self.loss, self.print_op],
                                         feed_dict={self.X_input: batch_images, self.X_input_inv: batch_images_inv, self.lr: lr[epoch], self.is_training: True})
...
####################################################################   
    ...
    def test(self, test_files, test_set, ckpt_dir, save_dir):
        ...
        for idx in range(len(test_files)):
            real_image = load_sar_images(test_files[idx]).astype(np.float32)  # T1
            real_image_channels = np.zeros((1, 256, 256, self.input_c_dim))
            real_image_channels_inv = np.zeros((1, 256, 256, self.input_c_dim))


            ## PILE
            for channel_i in range(self.input_c_dim-1):
                pile_im = load_sar_images(test_files[(idx+1)%len(test_files)]).astype(np.float32)  # WE TAKE TIME T2 THAT IS THE NEXT IMAGE IN THE LIST.
                real_image_channels = np.concatenate((real_image, pile_im), axis=3)                     ##T1T2 STACK
                real_image_channels_inv = np.concatenate((pile_im, real_image), axis=3)              ##T2T1 STACK
            ...
            for x in x_range:
                for y in y_range:
                    tmp_clean_image, map, map2, noisy_image = self.sess.run([self.Y, self.Y2, self.Y2_2, self.X],
                                                                 feed_dict={self.X_input: real_image_channels[:, x:x + pat_size,
                                                                                     y:y + pat_size, :], 
                                                                            self.X_input_inv: real_image_channels_inv[:, x:x + pat_size,
                                                                                     y:y + pat_size, :],
                                                                            self.is_training: False})
            ...
            noisyimage2 = denormalize_sar(real_image_channels[:,:,:,-1:])
            outputimage = denormalize_sar(output_clean_image - cn)

            imagename = test_files[idx].replace(test_set, "")
            save_sar_images(outputimage, noisyimage, imagename, save_dir)
            imagename2 = test_files[idx].replace(test_set, "_2_")
            save_mapbm(noisyimage2, imagename2, save_dir)

            imagename_map = test_files[idx].replace(test_set, "_map1_")
            save_mapbm(map[0,:,:,0]*255, imagename_map, save_dir)

            imagename_map1 = test_files[idx].replace(test_set, "_map2_")
            save_mapbm(map2[0,:,:,0]*255, imagename_map1, save_dir)
