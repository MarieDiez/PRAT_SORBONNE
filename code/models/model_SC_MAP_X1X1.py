...
class denoiser(object):
    def __init__(self, sess, stride=128, input_c_dim=1, batch_size=4):
        ...
        self.loss = self.alpha*((1.0 / batch_size) * tf.reduce_sum( tf.abs( self.X_input[:,:,:,:1] - self.Y+cn))) + self.beta * ((1.0/batch_size)*tf.reduce_sum(tf.abs(self.Y2 - self.X_input[:,:,:,:1] + self.X_input[:,:,:,-1:])))
    ...
    def evaluate(self, iter_num, test_data, eval_files, eval_set, sample_dir): 
        ...
        for idx, current_test in enumerate(test_data):
            ...
            for channel_i in range(self.input_c_dim-1):
                pile_im = test_data[(idx+1)%len(test_data)] # WE TAKE THE NEXT IMAGE ON THE LIST THAT IS THE SAME IMAGE TIME WITH ANOTHER NOISE SIMULATION 
                # NOISY IMAGES NEED TO BE THE SAME WITH 2 DIFFERENT SIMULATIONS OF NOISE IN THE FOLDER  CORRESPONDING  TO THE EVALUATION (FOR A PILE OF SIZE 2)
                real_image_channels = np.concatenate((real_image_channels, pile_im[:,:256,:256,:]), axis=3)

            output_clean_image, output_clean_image2, noisy_image = self.sess.run(
                [self.Y, self.Y2, self.X],
            ...
            imagename_map = imagename0.replace('.npy', '_map_' + str(iter_num) + '.npy')
            save_mapbm(output_clean_image2[0,:,:,0]*255, imagename_map, sample_dir)
            noisy_name2 = imagename0.replace('.npy', '_noisy2_' + str(iter_num) + '.npy')
            save_mapbm(noisyimage2, noisy_name2, sample_dir)
            ...
            for batch_id in range(start_step, numBatch):
                ...
                for i in range(batch_size):
                    ...
                    if self.input_c_dim == 2 :
                        im1 = data[id_pile][1][ x:x + pat_size, y:y + pat_size, id_date] # WE TAKE THE SAME NOT NOISY IMAGE, ON THOSE WE WILL ADD NOISE WITH true_fn FUNCTION. 
                        im_channels = np.concatenate((im_channels, np.expand_dims(im1, axis=2)), axis=2)
                ...
                print("Epoch: [%2d] [%4d/%4d] time: %4.4f, loss: %.6f"
                      % (epoch + 1, (batch_id + 1) , numBatch, time.time() - start_time, loss)) # affichage
...
####################################################################   
    ...
    def test(self, test_files, test_set, ckpt_dir, save_dir):
        ...
        for idx in range(len(test_files)):
            ...
            ## PILE X1X1
            for channel_i in range(self.input_c_dim-1):
                pile_im = load_sar_images(test_files[(idx+1)%len(test_files)]).astype(np.float32) # WE TAKE THE NEXT IMAGE ON THE LIST THAT IS THE SAME IMAGE TIME WITH ANOTHER NOISE SIMULATION 
                # NOISY IMAGES NEED TO BE THE SAME WITH 2 DIFFERENT SIMULATIONS OF NOISE IN THE FOLDER  CORRESPONDING  TO THE TEST (FOR A PILE OF SIZE 2)
                real_image_channels = np.concatenate((real_image, pile_im), axis=3)
            ...
            for x in x_range:
                for y in y_range:
                    tmp_clean_image, map, noisy_image = self.sess.run([self.Y, self.Y2, self.X],
            ...
            imagename_map = test_files[idx].replace(test_set, "_map_")
