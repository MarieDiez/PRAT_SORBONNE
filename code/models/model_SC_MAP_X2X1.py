...
class denoiser(object):
    def __init__(self, sess, stride=128, input_c_dim=1, batch_size=4):
        ...
        self.loss = self.alpha*((1.0 / batch_size) * tf.reduce_sum( tf.abs( self.X_input[:,:,:,:1] - self.Y+cn))) + self.beta * ((1.0/batch_size)*tf.reduce_sum(tf.abs(self.Y2 - self.Y + self.X_input[:,:,:,-1:])))
...
####################################################################   
    ...
    def evaluate(self, iter_num, test_data, eval_files, eval_set, sample_dir): 
        ...
        for idx, current_test in enumerate(test_data):
            ...
            pile_im = real_image[:,:256,:256,:].copy() # T2 IMAGE
            idx_lst=[idx]

            ## PILE X2X1
            for channel_i in range(self.input_c_dim-1):
                pile_im = test_data[(idx+1)%len(test_data)] # WE TAKE TIME T1 THAT IS THE NEXT IMAGE IN THE LIST.
                real_image_channels = np.concatenate((pile_im[:,:256,:256,:], real_image_channels), axis=3) # T2T1 STACK 
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
                ...
                for i in range(batch_size):
                    ...
                    im1 = im0.copy() 
                    im_channels=im0
                    im_channels = np.expand_dims(im_channels, axis=2) # T2 IMAGE

                    ## PILE X2X1
                    if self.input_c_dim == 2 :
                        im1 = data[id_pile][1][ x:x + pat_size, y:y + pat_size, (id_date+1)%data[id_pile][1].shape[-1]] # WE TAKE TIME T1 THAT IS THE NEXT IMAGE IN THE LIST.
                        im_channels = np.concatenate((np.expand_dims(im1, axis=2), im_channels),  axis=2) # T2T1 STACK
...
####################################################################   
    ...
    def test(self, test_files, test_set, ckpt_dir, save_dir): 
        ...
        for idx in range(len(test_files)):
            ...
            real_image_channels = np.zeros((1, 256, 256, self.input_c_dim)) # T2 IMAGE

            ## PILE X2X1
            for channel_i in range(self.input_c_dim-1): 
                pile_im = load_sar_images(test_files[(idx+1)%len(test_files)]).astype(np.float32)  # WE TAKE TIME T1 THAT IS THE NEXT IMAGE IN THE LIST.
                real_image_channels = np.concatenate((pile_im, real_image), axis=3)  # T2T1 STACK
